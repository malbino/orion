...
public class CentralizadorCalificacionesFacade {
    ...
    public List<Estudiante> listaEstudiantes(int id_gestioncademica, int id_carrera, Nivel nivel, Turno turno) {
        List<Estudiante> l = new ArrayList();

        try {
            Query q = em.createQuery("SELECT DISTINCT e FROM Nota n JOIN n.gestionAcademica ga JOIN n.materia m JOIN m.carrera c JOIN n.grupo g JOIN n.estudiante e WHERE ga.id_gestionacademica=:id_gestionacademica AND c.id_carrera=:id_carrera AND m.nivel=:nivel AND g.turno=:turno AND m.curricular=:curricular AND n.modalidad=:modalidad ORDER BY e.primerApellido, e.segundoApellido, e.nombre");
    ...
    @Transactional(Transactional.TxType.REQUIRED)
    public List<Estudiante> listaEstudiantesRecuperatorio(int id_gestioncademica, int id_carrera, Nivel nivel, Turno turno) {
        List<Estudiante> l = new ArrayList();

        try {
            Query q = em.createQuery("SELECT DISTINCT e FROM Nota n JOIN n.gestionAcademica ga JOIN n.materia m JOIN m.carrera c JOIN n.grupo g JOIN n.estudiante e WHERE ga.id_gestionacademica=:id_gestionacademica AND c.id_carrera=:id_carrera AND m.nivel=:nivel AND g.turno=:turno AND m.curricular=:curricular AND n.modalidad=:modalidad AND n.recuperatorio IS NOT NULL ORDER BY e.primerApellido, e.segundoApellido, e.nombre");
            q.setParameter("id_gestionacademica", id_gestioncademica);
            q.setParameter("id_carrera", id_carrera);
            q.setParameter("nivel", nivel);
            q.setParameter("turno", turno);
    ...
    @Transactional(Transactional.TxType.REQUIRED)
    public List<Nota> listaNotas(int id_gestioncademica, int id_carrera, Nivel nivel, Turno turno) {
        List<Nota> l = new ArrayList();

        try {
            Query q = em.createQuery("SELECT n FROM Nota n JOIN n.gestionAcademica ga JOIN n.materia m JOIN m.carrera c JOIN n.grupo g JOIN n.estudiante e WHERE ga.id_gestionacademica=:id_gestionacademica AND c.id_carrera=:id_carrera AND m.nivel=:nivel AND g.turno=:turno AND m.curricular=:curricular AND n.modalidad=:modalidad ORDER BY e.primerApellido, e.segundoApellido, e.nombre, m.numero");
            q.setParameter("id_gestionacademica", id_gestioncademica);
            q.setParameter("id_carrera", id_carrera);
            q.setParameter("nivel", nivel);
            q.setParameter("turno", turno);
    ...
    public Centralizador centralizadorCalificaciones(GestionAcademica gestionAcademica, Carrera carrera, int numeroLibro) {
        ...
        for (Nivel nivel : niveles) { //niveles
            ...
            for (Turno turno : turnos) { //turnos
                ...
                List<Estudiante> estudiantes = listaEstudiantes(gestionAcademica.getId_gestionacademica(), carrera.getId_carrera(), nivel, turno);
                ...
                if (!estudiantes.isEmpty()) {
                    ...
                    //notas
                    List<Nota> notas = listaNotas(gestionAcademica.getId_gestionacademica(), carrera.getId_carrera(), nivel, turno);
        ...
        for (Nivel nivel : niveles) { //niveles
            ...
            for (Turno turno : turnos) { //turnos
                ...
                List<Estudiante> estudiantes = listaEstudiantesRecuperatorio(gestionAcademica.getId_gestionacademica(), carrera.getId_carrera(), nivel, turno);
                Iterator<Estudiante> iteratorEstudiantes = estudiantes.iterator();
                if (!estudiantes.isEmpty()) {
                    //materias centralizador
                    List<Materia> materias = listaMaterias(carrera.getId_carrera(), nivel);
