 ...
 * @author Tincho
 */
@Named("ReporteCartaSolicitudController")
@SessionScoped
public class ReporteCartaSolicitudController extends AbstractController implements Serializable {

    private static final String PATHNAME = File.separator + "resources" + File.separator + "uploads" + File.separator + "carta_solicitud.docx";
    ...
    public void bajarCarta(NotaPasantia notaPasantia) throws IOException {
        ...
        for (XWPFParagraph p : document.getParagraphs()) {
            ...
            if (runs != null) {
                for (XWPFRun r : runs) {
                    ...
                    if (text != null && text.contains("<<NOMBRE_CONTACTO>>")) {
                        text = text.replace("<<NOMBRE_CONTACTO>>", notaPasantia.getEmpresa().nombreContacto());
                        r.setText(text, 0);
                    }
                    ...
                    if (text != null && text.contains("<<DNI_ESTUDIANTE>>")) {
                        text = text.replace("<<DNI_ESTUDIANTE>>", notaPasantia.getEstudiante().dniLugar());
                        r.setText(text, 0);
                    }
        ...
        //log
        logFacade.create(new Log(Fecha.getDate(), EventoLog.READ, "Generación reporte carta de solicitud", loginController.getUsr().toString()));
    }
