...
public class ListaPostulantes extends HttpServlet {
    ...
    public void generarPDF(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (id_gestionacademica != null && id_carrera != null) {
            ...
            try {
                ...
                Logger.getLogger(ListaPostulantes.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    public PdfPTable titulo(GestionAcademica gestionAcademica, Carrera carrera) throws BadElementException, IOException {
        ...
        cell = new PdfPCell(new Phrase("LISTA POSTULANTES,", TITULO));
    ...
    public PdfPTable contenido(GestionAcademica gestionAcademica, Carrera carrera) throws BadElementException, IOException {
        ...
        List<Postulante> listaPostulantes = postulanteFacade.listaPostulantesGACarrera(gestionAcademica.getId_gestionacademica(), carrera.getId_carrera());
        for (int i = 0; i < listaPostulantes.size(); i++) {
            ...
            cell = new PdfPCell(new Phrase(String.valueOf(postulante.getCodigo()), NORMAL));
            cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
            cell.setVerticalAlignment(PdfPCell.ALIGN_MIDDLE);
            cell.setColspan(10);
            table.addCell(cell);

            cell = new PdfPCell(new Phrase(postulante.toString(), NORMAL));
            cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
            cell.setVerticalAlignment(PdfPCell.ALIGN_MIDDLE);
            cell.setColspan(35);
            table.addCell(cell);

            cell = new PdfPCell(new Phrase(String.valueOf(postulante.dniLugar()), NORMAL));
            cell.setHorizontalAlignment(PdfPCell.ALIGN_CENTER);
            cell.setVerticalAlignment(PdfPCell.ALIGN_MIDDLE);
            cell.setColspan(10);
            table.addCell(cell);

            if (postulante.getCelular() != null) {
                cell = new PdfPCell(new Phrase(String.valueOf(postulante.getCelular()), NORMAL));
            } else {
